...
test('add extension to the redbone', () => {
  const redbone = new Redbone(new Connector());
  const extension = { name: 'test' };
  redbone.extension(extension);
  expect(redbone.extensions.test).toBe(extension);
  expect(redbone.extensions.test.redbone).toBe(redbone);
});

test('add invalid extension to the redbone', () => {
  const redbone = new Redbone(new Connector());
  const extension = {};
  expect(() => redbone.extension(extension)).
  toThrow(new TypeError('extension name is not defined'));
});
