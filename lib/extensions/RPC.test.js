...
test('pubsub of rpc', (cb) => {
  ...
  const ee = new MyEE();
  rpc.setModule('events', ee);
  const client = new Client();
  let counter = 0;
  client.dispatcher = (action) => {
    if (action.type === rpc.types['client/rpc'].SUBSCRIBED) {
      expect(ee.listenerCount('ping')).toBe(1);
      counter += 1;
    }
    if (action.type === 'TEST') {
      expect(action.value === 'pong');
      counter += 2;
    }
    if (action.type === rpc.types['client/rpc'].UNSUBSCRIBED) {
  ...
  process.nextTick(() => {
    ee.emit('ping', { value: 'pong' });
    process.nextTick(() => {
      redbone.onDispatch(client, {
        type: rpc.types['server/rpc'].UNSUB,
        id: 2,
        module: 'events',
        event: 'ping',
        echo: true
