...
dispatcher.watch = function watch(type, fn) {
  ...
  if (!dispatcher.__watchers[type]) dispatcher.__watchers[type] = [];
  dispatcher.__watchers[type].push(fn);
  return dispatcher
};
...
dispatcher.__callWatchers = function(socket, action) {
  ...
  const watchers = dispatcher.__watchers[action.type];
  if (!watchers) return;
  try {
